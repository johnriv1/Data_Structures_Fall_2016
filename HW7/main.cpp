...
typedef std::map<std::string, std::vector<PersonClass> > COSTUME_PEOPLE_TYPE;
...
void rentCostume(COSTUME_SHOP_TYPE& costume_shop, PEOPLE_TYPE& people, COSTUME_PEOPLE_TYPE& rented_costumes) {
  ...
  //person already renting a different costume, so return that costume and rent the new one
	...
	//remove the person from the vector corresponding to the previous costume they were renting.
	rented_costumes[prevCostume].erase(std::remove(rented_costumes[prevCostume].begin(), rented_costumes[prevCostume].end(), curr_person), rented_costumes[prevCostume].end());
...
void lookup(COSTUME_SHOP_TYPE& costume_shop, COSTUME_PEOPLE_TYPE& rented_costumes) {
  std::string costume_name;
  std::vector<PersonClass> people_renting;
  ...
  if (costume_shop.find(costume_name) == costume_shop.end()) {
	  ...
	  if (rented_costumes[costume_name].size() > 0) {
		  ...
		  for (int i = 0; i < rented_costumes[costume_name].size(); i++) {
			  std::cout << "    " << rented_costumes[costume_name][i].get_fname() << " " << rented_costumes[costume_name][i].get_lname() << std::endl;
		  }
	  }
  }
}
