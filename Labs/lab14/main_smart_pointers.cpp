...
void deleteAll1(Balloon* ptr, std::set<Balloon*> &set) {
	for (int j = 0; j < (ptr->numRopes()); j++) {
		std::pair<std::set<Balloon*>::iterator, bool> check = set.insert(ptr->getRope(j));
		if (check.second == false) {
			return;
		}
		else {
			deleteAll1(ptr->getRope(j), set);
		}
	}
}

void deleteAll(Balloon* ptr) {
	std::set<Balloon*> set;
...
/*
  ...
  dsSharedPtr isabelle(new Balloon("Shrek"));
  henry->addRope(isabelle);
  isabelle = new Balloon("Barney the Purple Dinosaur");
  georgette->addRope(isabelle);
