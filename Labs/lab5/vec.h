...
public:   
  ...
  //size_type remove_matching_elements(Vec<T>& v, const T& val);
  void print() const;
  void halfArray();
...
template <class T> void Vec<T>::halfArray() {
	if (m_size < m_alloc/2) {
		m_alloc = m_size;
		T* new_data = new T[m_alloc];
		for (size_type i = 0; i < m_size; ++i) {
...
template<class T>
typename Vec<T>::size_type remove_matching_elements(Vec<T>& v, const T& val);
