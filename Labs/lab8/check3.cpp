...
void print_grid(vector<vector<bool> > const& blocked_grid, int start_x, int start_y) {
  ...
  int toOrigin(int x, int y, vector<vector<bool> > const& blocked_grid, string final_string) { 
	if (x==0 && y==0) {
		...
		string tmp= "(" + to_string(x) + "," + to_string(y) + ") ";
		final_string += tmp;
		return toOrigin(x, y-1,blocked_grid, final_string);
	}
	else if (x!=0 && y==0) {
		string tmp = "(" + to_string(x) + "," + to_string(y) + ") ";
		final_string += tmp;
		return toOrigin(x-1, y,blocked_grid, final_string);
	}
	final_string += "(" + to_string(x) + "," + to_string(y) + ") ";
	int Count = toOrigin(x-1, y,blocked_grid, final_string);
	if (Count == 0) {
		Count = toOrigin(x, y-1, blocked_grid, final_string);
	}
	return Count;
  }
...
int main(int argc, char* argv[]) {
  ...
  string final_string = "";
  cout << toOrigin(start_x, start_y, blocked_grid, final_string) << endl;
  return 0;
}