...
void print_grid(vector<vector<bool> > const& blocked_grid, int start_x, int start_y) {
  ...
  int toOrigin(int x, int y, vector<vector<bool> > const& blocked_grid) { 
	if (x==0 && y==0) {
		return 1;
	}
	else if (blocked_grid[x][y]) {
		...
		return toOrigin(x, y-1,blocked_grid);
	}
	else if (x!=0 && y==0) {
		return toOrigin(x-1, y,blocked_grid);
	}
	return toOrigin(x-1, y,blocked_grid) + toOrigin(x, y-1, blocked_grid);
  }
...
int main(int argc, char* argv[]) {
  ...
  std::cout << toOrigin(start_x, start_y, blocked_grid) << std::endl;
  return 0;
}