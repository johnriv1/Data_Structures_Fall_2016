...
int main() {
  ...
  for (int a=0; a<5; a++) {
    ...
    float root_1, root_2;
    bool success = find_roots(1, b_array[a],c_array[a], root_1,root_2);
	...
	//find smallest root and store it
	if (root_1 >= root_2) {
		smaller_root[a] = root_2;
	}
	else {
		smaller_root[a] = root_1;
	}
	
    // Check our work...
	/*
    if (check_root(1,b_array[a],c_array[a], root_1) && check_root(1,b_array[a],c_array[a], root_2)) {
