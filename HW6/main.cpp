...
int Stroke(int x, int y, Board board, std::string final_string,  int count, char val, int width, int  height) { 
	...
	//or, if path tries to go through itself or through another stroke, return 0
		...
		final_string += tmp;
		count += 1;
		return Stroke(x, y-1,blocked_grid, final_string) + Stroke(x, y+1,blocked_grid, final_string)+ Stroke(x+1, y,blocked_grid, final_string);
	}
	else if (x==0 && y!=0 && y == (height-1) {
		string tmp = "(" + to_string(x) + "," + to_string(y) + ") ";
		final_string += tmp;
		count += 1;
		return Stroke(x, y-1,blocked_grid, final_string) + Stroke(x+1, y,blocked_grid, final_string);
	}
	else if (y==0 && x!=0 && x!=(width-1)) {
		string tmp = "(" + to_string(x) + "," + to_string(y) + ") ";
		final_string += tmp;
		count += 1;
		return Stroke(x-1, y,blocked_grid, final_string) + Stroke(x+1,blocked_grid, final_string)+ Stroke(x, y+1,blocked_grid, final_string);
	}
	else if (y==0 && x!=0 && x==(width-1)) {
		string tmp = "(" + to_string(x) + "," + to_string(y) + ") ";
		final_string += tmp;
		count += 1;
		return Stroke(x-1, y,blocked_grid, final_string) + Stroke(x, y+1,blocked_grid, final_string);
	}
	else if (y==0 && x==0) {
		string tmp = "(" + to_string(x) + "," + to_string(y) + ") ";
		final_string += tmp;
		...
		count += 1;
		return Stroke(x, y-1,blocked_grid, final_string) + Stroke(x, y+1,blocked_grid, final_string)+ Stroke(x+1, y,blocked_grid, final_string) + Stroke(x-1, y,blocked_grid, final_string);
	}
