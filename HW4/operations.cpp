...
float multidivide(int numerator, int d1, int d2, int d3, int d4) {
  float f = ((((numerator / float(d1)) / float(d2)) / float(d3)) / float(d4));
  return f;
}
...
int arithmetic_operations() {
  ...
  int f = 2*b + 2*c;                                           //  100
  int g = e - (b/c) + d + 20;                                  // -1 
  int h = int(ceil((float(f)/float(c)) / float(a) ));          //  3 
  int m = (d / h) / 7;                                         // -2 
  int n = g + m;                                               // -3 
  int p = (f / e) - h - 1;                                     // -1 
  int q = f + 2*d;                                             // 16
...
int pythagoras(int x, int y) {
  ...
  if((fracpart == 0)) {
	returnval = (int) *placeholder;
	delete placeholder;
    return returnval;
  }

  // x is the hypotenuse, need to subtract instead of add
  float diffsquares1 = y*y - x*x;
  if (diffsquares1 >= 0) {  
	  fracpart = modf(sqrt(diffsquares1), placeholder);
	  if((fracpart == 0)) {
		returnval = (int) *placeholder;
		delete placeholder;
		return returnval;
