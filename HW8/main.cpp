...
void StudentTests() {
  ...
  //
	...
	bidirectional_map<int,char> bi1;
	bi1.insert(std::make_pair(2, 'q'));
	bi1.insert(std::make_pair(3, 'a'));
	bi1.insert(std::make_pair(1, 'z'));
	bi1.print(std::cout);
	bidirectional_map<int,char> bi2;
	bi2.insert(std::make_pair(9, 't'));
	bi2.insert(std::make_pair(7, 'd'));
	bi2.insert(std::make_pair(0, 'n'));
	bi2.print(std::cout);
	std::cout << "DIFFERENT MAP TYPES TEST SUCCESSFUL" << std::endl;
	std::cout << std::endl;
	
	std::cout << "COPY CONSTRUCTOR" << std::endl;
	bidirectional_map<int, char> bi1copy(bi1);
	//should print the same thing.
	std::cout << "BEFORE BI1COPY INSERTS" << std::endl;
	std::cout << "BI1:" << std::endl;
	bi1.print(std::cout);
	std::cout << "BI1COPY:" << std::endl;
	bi1copy.print(std::cout);
	bi1copy.insert(std::make_pair(12, 'y'));
	bi1copy.insert(std::make_pair(4, 'o'));
	//should be different
	std::cout << "AFTER BI1COPY INSERTS" << std::endl;
	std::cout << "BI1:" << std::endl;
	bi1.print(std::cout);
	std::cout << "BI1COPY:" << std::endl;
	bi1copy.print(std::cout);
	std::cout << "COPY CONSTRUCTOR TEST SUCCESSFUL" << std::endl;
	std::cout << std::endl;
	
	std::cout << "ASSIGNEMENT OPERATOR" << std::endl;
	bidirectional_map<int, char> bm = bi1copy;
	bm.print(std::cout);
	std::cout << "ASSIGNMENT OPERATOR TEST SUCCSEESFUL" << std::endl;
	
	std::cout << "FIND FUNCTION" << std::endl;
	bidirectional_map<std::string,int>::value_iterator vi;
	assert (bm.find(12) != bm.key_end());
	assert (bm.find('z') != bm.value_end());
	assert (bm.find(5) == bm.key_end());
	assert (bm.find('b') == bm.value_end());
	std::cout << "FIND FUNCTION TEST COMPLETED" << std::endl;
	
	std::cout << "INSERT FUNCTION" << std::endl;
	std::pair<bidirectional_map<int , char>::key_iterator,bool> insert_return;
	
	//attempt to insert already existing key and value
    insert_return = bm.insert(std::make_pair(1, 'z'));
	assert (insert_return.first == bm.find(1));
	assert (insert_return.second == false);
	
	//attempt to insert already existing key only
	insert_return = bm.insert(std::make_pair(12, 'x'));
	assert (insert_return.first == bm.find(12));
	assert (insert_return.second == false);

	assert (bm.size() == 5);
	bm.print(std::cout);
	
	//insert valid key and value
	insert_return = bm.insert(std::make_pair(5, 'x'));
	assert (insert_return.first == bm.find(5));
	assert (insert_return.second == true);
	
	assert (bm.size() == 6);
	bm.print(std::cout);
	
	std::cout << "INSERT TEST SUCCESSFUL" << std::endl;
	
	std::cout << "ERASE FUNCTION" << std::endl;
	//try to erase key that doesnt exist
	assert (bm.erase(6) == 0);
	/*
	std::cout << "ERASE 2 (ROOT NODE)" << std::endl;
	assert (bm.erase(2) == 1);
	bm.print(std::cout);
	*/
	std::cout << "ERASE 5 (LEAF NODE)" << std::endl;
	//erase leaf node
	assert (bm.erase(5) == 1);
	bm.print(std::cout);
	std::cout << "ERASE 12 (NEITHER ROOT NOR LEAF NODE)" << std::endl;
	assert (bm.erase(12) == 1);
	bm.print(std::cout);
	//erase from empty bidirectional map
	bidirectional_map<std::string,int> bdmap;
	assert (bdmap.erase("hI") == 0);
	  
	std::cout << "ERASE TEST COMPLETED" << std::endl;
